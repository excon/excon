    ...
    tests('persistent connections') do
      ...
      ip_ports.each do |ip_port|
        tests("with default :persistent => true, #{ip_port}") do
          ...
          returns(%w[1 2], 'uses a persistent connection') do
          ...
          returns(%w[3 1 2], ':persistent => false resets connection') do
            ret = []
        ...
        tests("with default :persistent => false, #{ip_port}") do
          ...
          returns(%w[1 1], 'does not use a persistent connection') do
          ...
          returns(%w[1 2 3 1], ':persistent => true enables persistence') do
            ret = []
            ret << connection.request(method:     :get,
                                      path:       '/echo/request_count',
